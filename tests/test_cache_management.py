...
def test_lru_logic():
    ...
    config = {"cache": {"enabled": True, "max_cache_size_mb": 2}}
    ...
    for name in files:
        ...
        # B: now - 2h
        # C: now - 1h
        ts = (datetime.now() - timedelta(hours=3 - files.index(name))).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        with sqlite3.connect(test_db_path) as conn:
            conn.execute(
                "UPDATE pdb_cache SET last_accessed = ? WHERE id = ?", (ts, name)
            )
            conn.commit()
