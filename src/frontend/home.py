...
def render_hero_section():
    ...
    # 2. Key Metrics Bar
    try:
        stats = st.session_state.system_manager.get_aggregate_stats(st.session_state.history_db)
        ...
        with m_col3:
            # Estimate cache size
            cache_mb = st.session_state.history_db.get_total_cache_size() / (1024 * 1024)
    ...
    with dash_col1:
        ...
        if recent_runs:
            for idx, run in enumerate(recent_runs):
                with st.expander(f"📁 {run['name']} — {run['timestamp']}", expanded=False):
                    c1, c2 = st.columns([2, 1])
                    with c1:
                        st.write(f"**Proteins**: {', '.join(run['pdb_ids'])}")
                    with c2:
                        if st.button(f"🚀 Load Run", key=f"load_{run['id']}_{idx}", use_container_width=True):
            ...
            st.info("No analysis history found yet. Start your first run from the sidebar!")
    ...
    with dash_col2:
        ...
        for name, ids in EXAMPLES.items():
            if st.button(f"🧬 Compare {name}", help=f"IDs: {', '.join(ids[:3])}...", use_container_width=True):
...
- SQLite (History)
    ...
    with v_col1:
        st.info("📊 **Rigorous Metrics**: Pairwise RMSD matrices and hierarchical clustering.")
    with v_col2:
        st.info("🌳 **Evolutionary Insights**: Structural phylogeny trees generated from alignment scores.")
