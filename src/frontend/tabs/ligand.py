...
def render_ligand_tab(results: Dict[str, Any]) -> None:
        ...
        if pdb_path:
            ...
            if not ligands:
                ...
                with sel_col2:
                    ...
                    if st.button(
                        "Analyze Interactions", type="primary", use_container_width=True
                    ):
                        interactions = (
                            st.session_state.ligand_analyzer.calculate_interactions(
            ...
            if "error" in interactions:
                ...
                with res_col2:
                    ...
                    if interactions["interactions"]:
                        ...
                        st.dataframe(
                            df_int[
                                ["residue", "chain", "resi", "distance", "type"]
    ...
    with tab_compare:
        ...
        if len(history) < 2:
            ...
            st.caption(
                "Jaccard Index based on shared residue types in the binding pocket."
            )

            sim_matrix = (
                st.session_state.ligand_analyzer.calculate_interaction_similarity(
                    history
                )
            )
            st.dataframe(
                sim_matrix.style.background_gradient(cmap="Greens", vmin=0, vmax=1)
            )
            ...
            if l1_id and l2_id:
                ...
                delta_col1.metric(
                    "Shared Residue Types", len(shared), help=f"{', '.join(shared)}"
                )
                delta_col2.metric(
                    f"Unique to {l1_id}", len(unique1), help=f"{', '.join(unique1)}"
                )
                delta_col3.metric(
                    f"Unique to {l2_id}", len(unique2), help=f"{', '.join(unique2)}"
                )
    ...
    with tab_sasa:
        ...
        sasa_pdb_path = None
        for name in [
            f"{selected_sasa_pdb}.pdb",
            f"{selected_sasa_pdb.lower()}.pdb",
            f"{selected_sasa_pdb.upper()}.pdb",
        ]:
            p = result_dir / name
            if p.exists():
                sasa_pdb_path = p
        ...
        if sasa_pdb_path:
            if st.button(
                ...
                with st.spinner("Computing solvent accessible surface area..."):
                    sasa_result = st.session_state.ligand_analyzer.calculate_sasa(
                        sasa_pdb_path
                    )
            ...
            if (
                ...
                if "error" in sasa:
                    ...
                    chain_summary = ", ".join(
                        [
                            f"Chain {k}: {v:.0f} Å²"
                            for k, v in sasa["chain_sasa"].items()
                        ]
                    )
                    ...
                    if sasa.get("residues"):
                        ...
                        df_sasa["label"] = df_sasa["residue"] + df_sasa["resi"].astype(
                            str
                        )
