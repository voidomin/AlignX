...
def render_clusters_tab(results: Dict[str, Any]) -> None:
    ...
    clusters = st.session_state.rmsd_analyzer.identify_clusters(
        rmsd_df, threshold=threshold
    )

    if clusters:
        st.markdown(
            f"Found **{len(clusters)}** distinct structural families at **{threshold} Å** cutoff."
        )
        ...
        for cid, members in clusters.items():
            ...
            with st.expander(
                f"📁 Cluster {cid} ({len(members)} members, Avg RMSD: {avg_rmsd:.2f} Å)",
                expanded=True,
            ):
                member_data = []
                for m in members:
                ...
                # View in 3D Button
                if st.button(
                    f"🎯 View Cluster {cid} in 3D",
                    key=f"btn_view_cluster_{cid}",
                    use_container_width=True,
                ):
                    st.session_state.selected_cluster_members = members
                    st.session_state.show_3d_viewer = True
                    st.success(
                        f"Filter applied for Cluster {cid}. Switch to '3D Visualization' tab to view."
                    )
    else:
