...
def _prepare_ramachandran_data(torsion_data: Dict[str, Any], pdb_ids: list) -> Any:
    ...
    chain_map = {chr(65 + i): pid for i, pid in enumerate(pdb_ids)}

    all_data = (
        pd.concat(torsion_data.values(), keys=torsion_data.keys())
        .reset_index(level=0)
        .rename(columns={"level_0": "chain"})
    )
    all_data["protein_name"] = (
        all_data["chain"].map(chain_map).fillna(all_data["chain"])
    )
    return all_data
...
def _render_ramachandran_controls(protein_names: list) -> Tuple[list, bool, bool]:
    ...
    with col_ctrl2:
        ...
        show_all = st.toggle(
            "Show Residue Labels",
            value=False,
            help="Display residue names (e.g. GLY35) directly on the plot.",
        )
    with col_ctrl3:
        st.write("")  # Spacer
        show_regions = st.toggle(
            "Show Regions",
            value=True,
            help="Display shaded reference regions for Alpha-Helices and Beta-Sheets.",
        )
    return selected_proteins, show_all, show_regions


def _apply_highlight_opacity_and_size(all_data: Any, selected_proteins: list) -> None:
    all_data["opacity"] = 1.0
    all_data["size"] = 8
    if selected_proteins:
        all_data["opacity"] = all_data["protein_name"].apply(
            lambda x: 1.0 if x in selected_proteins else 0.15
        )
        all_data["size"] = all_data["protein_name"].apply(
            lambda x: 12 if x in selected_proteins else 6
        )


def _add_ramachandran_background_regions(fig) -> None:
    ...
    fig.add_vrect(
        ...
        line_width=0,
        layer="below",
        annotation_text="Beta-Sheet",
        annotation_position="top left",
    )


def _add_ramachandran_region_traces(fig, all_data: Any, show_all: bool) -> None:
    color_map = {
...
def _build_ramachandran_figure(all_data: Any, show_all: bool, show_regions: bool):
    import plotly.graph_objects as go

    fig = go.Figure()
    if show_regions:
        _add_ramachandran_background_regions(fig)
    _add_ramachandran_region_traces(fig, all_data, show_all)
    ...
    fig.update_layout(
        ...
        hoverlabel={"bgcolor": "#1E1E1E", "font_size": 12, "font_family": "monospace"},
    )
    return fig
...
def _render_ramachandran_plot(torsion_data: Dict[str, Any], pdb_ids: list) -> None:
    ...
    selected_proteins, show_all, show_regions = _render_ramachandran_controls(
        protein_names
    )
    _apply_highlight_opacity_and_size(all_data, selected_proteins)

    fig = _build_ramachandran_figure(all_data, show_all, show_regions)
...
def _render_ramachandran_summary_metrics(
    ...
    with m1:
        st.metric(
            "Favored Region %",
            f"{stats['favored_percent']:.1f}%",
            help="Higher is better. Typically >90% for high-resolution structures.",
        )
    with m2:
...
def render_phylo_tree_tab(results: Dict[str, Any]) -> None:
    ...
    _render_tree_visualization(results)
    ...
    if torsion_data:
        _render_ramachandran_plot(torsion_data, results.get("pdb_ids", []))
    else:
        st.info(
            "💡 Ramachandran analysis is currently computing or not available for this run."
        )

    _render_ramachandran_summary_metrics(results, has_torsion_data=bool(torsion_data))
