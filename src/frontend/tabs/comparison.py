...
def render_comparison_tab(current_results: dict):
    ...
    if len(all_runs) < 2:
        st.warning(
            "Not enough past runs found for comparison. You need at least two sets of results."
        )
        return
    ...
    with col2:
        ...
        options = {
            f"{r['timestamp']} - {r['id'][:8]}... ({r['protein_count']} p)": r["id"]
            for r in other_runs
        }
        selected_display = st.selectbox(
            "Select run to compare against:", options=list(options.keys())
        )
    ...
    if st.button("🚀 Run Comparative Analysis", type="primary"):
        with st.spinner("Calculating differences..."):
            ...
            if diff_df is None:
                st.error(
                    "No overlapping proteins found between these runs. Batch comparison requires at least one common protein (with the same name and chain selection) to calculate structural shifts."
                )
                return
            ...
            st.markdown(
                "Positive values (red) indicate increased divergence in the current run compared to the target."
            )

            # Get colormap from config
            cmap = st.session_state.config.get("visualization", {}).get(
            ...
            if diff_df.values.max() == 0 and diff_df.values.min() == 0:
                st.success(
                    "✨ **Perfect Consensus**: The structural relationships between the overlapping proteins are identical in both runs."
                )
            ...
            st.info(
                "Side-by-side 3D viewing with camera syncing is coming in the next update!"
            )
