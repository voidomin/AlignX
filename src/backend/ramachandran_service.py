...
class RamachandranService:
    ...
    def aggregate_metrics(self, torsion_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        """
        ...
        for chain_id, df in torsion_data.items():
            ...
            for _, row in chain_outliers.iterrows():
                outliers.append(f"{row['residue_name']}{row['residue_id']} (Chain {chain_id})")
                
        quality_score = (favored_count / total_residues * 100) if total_residues > 0 else 0
