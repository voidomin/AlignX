...
class CacheManager:
    ...
    def register_item(self, item_id: str, file_path: Path):
        ...
        if not os.path.exists(abs_path):
            logger.warning(f"Attempted to register non-existent file in cache: {abs_path}")
            return
    ...
    def enforce_limit(self):
        ...
        logger.info(f"Cache limit exceeded ({total_size / (1024*1024):.2f} MB > {self.limit_mb} MB). Evicting old items...")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'usage_percent': (total_size / (self.limit_mb * 1024 * 1024)) * 100 if self.limit_mb > 0 else 0
        }
