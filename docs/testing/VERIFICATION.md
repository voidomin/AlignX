...
npm test
```
*Expected Output:* all test files pass (currently 239 tests across the suite, covering `api.js` and every tab/panel component, including `DiscoverTab.js` and `SettingsTab.js`).
...
3. **Verify Full-Stack Single Port Execution (Recommended):**
   ...
   - Check the top bar's tab strip, now grouped into three sections separated by a thin divider: **Explore** (Overview, Discover), **Results** (Ligands, Sequence, Analytics, Clusters), **Workspace** (Diff Runs, History, Dashboard, Settings — "Diff Runs" was renamed from "Compare" to remove the collision with the Overview alignment workflow, which docs informally call a "Compare run"). At a normal window width, confirm small `‹`/`›` scroll buttons appear if not all 10 tabs fit, and that clicking any tab (including ones only reachable by scrolling) reliably navigates there — not to "New Workspace".
   ...
   - Add `1CA2` (human carbonic anhydrase II) — confirm a "N disordered regions" badge appears on its metadata line (a real gap in the deposited structure's residue numbering) with a tooltip listing the missing residue ranges. Add a real multi-model NMR entry - confirm an "NMR · N models (model 1 shown)" badge appears instead.
   ...
   - Add `1CA2` (real catalytic zinc) and select its `ZN` ligand — confirm the interaction table shows three `Metal Coordination` contacts (a distinct dot color) at real zinc-finger-scale distances (~1.9-2.1 Å) with residues `HIS94`, `HIS96`, `HIS119` — the actual, textbook-known coordination geometry of human carbonic anhydrase II's catalytic zinc.
   ...
   - On **Analytics**' Annotations sub-tab (using the `AF-P69905-F1` + `4HHB` run above — both resolve to the same real UniProt accession, P69905), confirm it resolves and displays real InterPro domains (e.g. "Globin") and GO terms (e.g. "oxygen carrier activity") for each structure via the picker, confirm a "Shared across all structures" section appears listing the domains/terms both structures have in common, and confirm switching to an `ESM-` structure shows a graceful "no UniProt accession could be resolved" message rather than an error. On the `AF-P69905-F1` structure specifically, confirm each domain shows a "Highlight in 3D" button (clicking it highlights that domain's real residue range in the 3D viewer); confirm `4HHB` (a plain PDB ID) does **not** show this button, since InterPro's UniProt-numbered domain locations aren't safe to use as a PDB entry's own author residue numbers without real SIFTS segment-mapping.
   - Run a second alignment with different structures, then check the **Diff Runs** tab to diff it against the first run.
...
10. **Verify the 3D viewer, ligand section, and the Overview cross-link:**
    Submit a ligand-bearing structure (e.g. `4HHB`) and confirm it renders in the 3D viewer (single-color, no RMSD HUD — this is a raw unaligned structure, not a Compare superposition) and the "Ligands & Binding Sites" section below finds and displays real interaction contacts for at least one ligand. Confirm a "Switch to Overview" nudge appears at the bottom of the results and actually navigates to the Overview tab when clicked (the reverse of Overview's own "Switch to Discover mode" nudge when fewer than 2 structures are selected).
11. **Verify the heuristic apo-pocket finder:**
    Submit `AF-P69905-F1` (hemoglobin alpha's AlphaFold model — has no bound ligand at all, unlike the real PDB entry). Confirm the ligand section shows a "Predicted binding pockets (no bound ligand found)" block instead of "No bound ligands detected", with a visible "computational prediction, not experimentally validated" disclaimer, at least one ranked candidate pocket with its residue list, and a working "Highlight in 3D" button per candidate. (A real run of this found a top candidate including `HIS88` plus a cluster of hydrophobic/aromatic residues — genuinely close to hemoglobin's real heme-binding pocket, which is a good sign the heuristic is picking up real signal, not just noise, but treat any individual result as illustrative, not definitive.)
