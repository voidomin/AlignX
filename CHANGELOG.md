...
### Added
- **Ligand chemistry lookup**: resolves a ligand's 3-letter code to real name/formula/SMILES/InChIKey via RCSB's Chemical Component Dictionary, cached the same way GO terms already are. Shown as a "what is this?" line in the Ligands tab once a ligand is selected.
- **Contact maps & difference-distance matrices**: a real CA-CA contact map for one structure, and a real difference-distance matrix between two structures over their commonly-aligned columns - reveals domain movements a single global RMSD hides. Computed on demand (not stored per-run, which would bloat run history with matrices that can reach ~200MB dense) via new `/api/contact-map`/`/api/difference-distance` endpoints, with a sparse fallback above a 3000-residue/column cap. New heatmaps in Analytics' RMSD Matrix sub-tab.
- **Druggability volume estimate**: each heuristic candidate pocket now gets a convex-hull volume estimate (still explicitly labeled heuristic, not a validated cavity volume). Also wires up the Ligands tab's "Candidate binding pockets" section, which had a working backend/API but no frontend consumer at all until now.
- **Real PDB-entry UniProt residue mapping**: closes a gap Phase 1 explicitly flagged - real PDB entries now get working "Highlight in 3D" for InterPro domains and UniProt features too, via a real per-segment SIFTS residue map (previously only AlphaFold's 1:1 numbering shortcut worked).
- **Mutation impact mapping + ClinVar**: maps a structure's own residue to its real UniProt position, then looks up the real wild-type residue/gene and (if a match exists) the real ClinVar clinical significance of a proposed substitution - also surfaces any already-known UniProt variant at that position. New "Map a mutation" UI in Analytics' Annotations sub-tab.
- **Literature links + run notes/tags**: each real PDB entry's card now shows a "View publication" link (PubMed/DOI) from RCSB's own primary-citation data. Runs can now carry free-text notes and tags (stored in the existing metadata column, no schema change) via a new inline editor in the History panel.
- **Bulk QC sweep**: a new "Run QC on all" button in the Workspace tab runs Ramachandran/secondary-structure/wwPDB-validation QC across every loaded structure at once, no alignment required - generalizes the existing per-card validation badge. Fixed a latent bug this exposed: the Ramachandran service hardcoded a PDB-only parser, silently returning nothing for AlphaFold's `.cif` downloads.
- **Documented REST API + real Jupyter export**: `/docs` (FastAPI's own Swagger UI, already live but previously unlinked) is now the linked API reference. Every run can also export as a real, runnable `.ipynb` notebook whose code cells re-fetch that run's data live from the REST API, unlike the existing static HTML snapshot.
- **True sequence-only MSA (EBI Clustal Omega)**: a real multiple sequence alignment computed purely from each loaded structure's own sequence, independent of Mustang's structural alignment - can disagree with it for divergent sequences sharing a similar fold. Runs as a background job (submit/poll/fetch, mirroring the existing Foldseek Discover pattern).
- **True evolutionary conservation (NCBI BLAST)**: searches for real homologs of a selected structure's sequence and scores real per-position conservation from their alignments (Shannon entropy) - genuinely different from the existing identity-across-loaded-structures approximation, which is now honestly labeled as such rather than implicitly claiming to be conservation. The longest-running job type in this app (real BLAST searches commonly take minutes).

### Verified
- Full backend suite: 1161 tests passing (up from 1017). `black`/`ruff` clean.
- Frontend suite: 373 Vitest tests passing (up from 312). `npm run lint` clean, `npm run build` succeeds.
- Real end-to-end verification against a live local server for every stage, including: real chemistry for HEM/RET ligands; a real 141-residue contact map and a real 4HHB-vs-AlphaFold difference-distance matrix; plausible convex-hull pocket volumes (400-700 Å³) for a real ligand-free AlphaFold structure; a UniProt position correctly translating to a different real PDB author residue (not an identity passthrough); the real sickle-cell mutation (4HHB chain B residue 6) correctly resolving to HBB position 7 with a real ClinVar record and the classic dbSNP:rs334 variant; a real PubMed/DOI link for 4HHB; real notes/tags persisting to and reading back from a real run; real Ramachandran/secondary-structure/validation stats for both a real PDB entry and an AlphaFold `.cif` structure; a downloaded `.ipynb` that validated against nbformat's schema and fully executed against the live server with zero errors; a real Clustal Omega job completing end-to-end with a real gapped alignment; and a real BLAST job completing end-to-end (~2.6 minutes) with 20 real homologs (including the actual human hemoglobin beta RefSeq) and a real conservation profile across all 147 query positions.
