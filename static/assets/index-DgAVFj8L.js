(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`http://127.0.0.1:8000`,t=null;function n(e){t=e}function r(e={}){return t?{...e,"X-API-Key":t}:e}function i(t,n){let r=new URL(t,e);if(n)for(let[e,t]of Object.entries(n))t!=null&&r.searchParams.set(e,t);return r.toString()}function a(e){if(!t)return e;let n=new URL(e);return n.searchParams.set(`api_key`,t),n.toString()}var o=/^\d[A-Z0-9]{3}$/,s=/^AF-[A-Z0-9]+-F\d+(-V\d+)?$/,c=/^SM-[A-Z0-9]+$/,l=/^ESM-MGYP\d+$/;function u(e){let t=(e||``).trim().toUpperCase();return o.test(t)||s.test(t)||c.test(t)||l.test(t)}var d=/^[A-Za-z0-9_-]+$/;function f(e,t){if(typeof e!=`string`||!d.test(e))throw Error(`Invalid ${t}: ${JSON.stringify(e)}`);return e}function p(e,t){if(!u(e))throw Error(`Invalid ${t}: ${JSON.stringify(e)}`);return e}async function ee(){let e=await fetch(i(`/health`),{priority:`low`});if(!e.ok)throw Error(`Health check failed`);return e.json()}async function te(e){let t=await fetch(i(`/api/suggest`,{q:e}),{headers:r()});if(!t.ok)throw Error(`Suggestions fetch failed`);return t.json()}async function m(e){let t=await fetch(i(`/api/chains`),{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:e})});if(!t.ok){let e=await t.json();throw Error(e.detail||`Chains fetch failed`)}return t.json()}async function h(e){let t=new FormData;t.append(`file`,e);let n=await fetch(i(`/api/upload`),{method:`POST`,headers:r(),body:t});if(!n.ok){let e=await n.json();throw Error(e.detail||`Upload failed`)}return n.json()}async function g(e,t,n,a){let o=await fetch(i(`/api/jobs/align`),{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:e,chain_selection:t,remove_water:n,remove_heteroatoms:a})});if(!o.ok){let e=await o.json();throw Error(e.detail||`Alignment submission failed`)}return o.json()}async function _(e){e=f(e,`jobId`);let t=await fetch(i(`/api/jobs/${e}`),{headers:r()});if(!t.ok){let e=await t.json();throw Error(e.detail||`Job status fetch failed`)}return t.json()}async function v(e,{intervalMs:t=1500,onTick:n=null}={}){for(;;){let r=await _(e);if(n&&n(r),r.status===`completed`||r.status===`failed`)return r;await new Promise(e=>setTimeout(e,t))}}async function y(e,t){let n={pdb_id:e};t&&t.length>0&&(n.databases=t);let a=await fetch(i(`/api/jobs/discover`),{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify(n)});if(!a.ok){let e=await a.json();throw Error(e.detail||`Discovery submission failed`)}return a.json()}async function b(e,t){let n=await fetch(i(`/api/clusters`),{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({rmsd_df:e,threshold:t})});if(!n.ok){let e=await n.json();throw Error(e.detail||`Clusters fetch failed`)}return n.json()}async function ne(e){e=e?f(e,`excludeRunId`):``;let t=await fetch(i(`/api/comparison/runs`,{exclude_run_id:e}),{headers:r()});if(!t.ok)throw Error(`Comparison runs fetch failed`);return t.json()}async function x(e,t){e=f(e,`currentRunId`),t=f(t,`targetRunId`);let n=await fetch(i(`/api/comparison`,{current_run_id:e,target_run_id:t}),{headers:r()});if(!n.ok){let e=await n.json();throw Error(e.detail||`Comparison fetch failed`)}return n.json()}async function S(e,t){e=p(e,`pdbId`),t=f(t,`runId`);let n=await fetch(i(`/api/ligands`,{pdb_id:e,run_id:t}),{headers:r()});if(!n.ok)throw Error(`Ligands fetch failed`);return n.json()}async function C(e,t,n){e=p(e,`pdbId`),t=f(t,`ligandId`),n=f(n,`runId`);let a=await fetch(i(`/api/interactions`,{pdb_id:e,ligand_id:t,run_id:n}),{headers:r()});if(!a.ok)throw Error(`Interactions fetch failed`);return a.json()}async function w(){let e=await fetch(i(`/api/memory`),{headers:r(),priority:`low`});if(!e.ok)throw Error(`Memory stats fetch failed`);return e.json()}async function T(){let e=await fetch(i(`/api/memory/clear`),{method:`POST`,headers:r()});if(!e.ok)throw Error(`Clear memory execution failed`);return e.json()}async function E(e){e=f(e,`runId`);let t=await fetch(i(`/api/runs/${e}`),{headers:r()});if(!t.ok){let e=await t.json();throw Error(e.detail||`Run fetch failed`)}return t.json()}function D(e){e=f(e,`runId`);let t=new URL(window.location.origin);return t.searchParams.set(`shared_run`,e),a(t.toString())}async function O(e=20,t=0){let n=await fetch(i(`/api/history`,{limit:e,offset:t}),{headers:r()});if(!n.ok)throw Error(`History fetch failed`);return n.json()}async function k(){let e=await fetch(i(`/api/stats`),{headers:r()});if(!e.ok)throw Error(`Stats fetch failed`);return e.json()}async function A(e){e=f(e,`runId`);let t=await fetch(i(`/api/sequence`,{run_id:e}),{headers:r()});if(!t.ok)throw Error(`Sequence alignment fetch failed`);return t.json()}function j(e){return e=f(e,`runId`),a(i(`/results/${e}/alignment.pdb`))}function M(e){return e=f(e,`runId`),a(i(`/results/${e}/alignment.fasta`))}var N=new Set([`summary`,`insights`,`heatmap`,`tree`,`matrix`]);function P(e,t){return e=f(e,`runId`),!t||t.length===0?a(i(`/api/report`,{run_id:e})):(t.forEach(e=>{if(!N.has(e))throw Error(`Invalid report section: ${JSON.stringify(e)}`)}),a(i(`/api/report`,{run_id:e,sections:t.join(`,`)})))}function F(e){return e=f(e,`runId`),a(i(`/api/notebook`,{run_id:e}))}function I(e){return e=f(e,`runId`),a(i(`/api/discover/report`,{run_id:e}))}function L(e){return e=f(e,`runId`),a(i(`/api/discover/export`,{run_id:e}))}var R=[{key:`dashboard`,label:`Dashboard`},{key:`overview`,label:`Overview`},{key:`discover`,label:`Discover`},{key:`ligands`,label:`Ligands`},{key:`sequence`,label:`Sequence`},{key:`analytics`,label:`Analytics`},{key:`clusters`,label:`Clusters`},{key:`comparison`,label:`Compare`},{key:`history`,label:`History`}],z=class{constructor(e){this.onTabChange=e.onTabChange,this.onExportData=e.onExportData,this.onNewWorkspace=e.onNewWorkspace,this.activeTab=`overview`,this.element=null,this.memoryInterval=null}render(){let e=document.createElement(`header`);return e.className=`sticky top-0 z-50 bg-surface border-b border-border shrink-0`,e.innerHTML=`
        ...
        `;try{let e=await x(this.currentRunId,this.targetRunId);this.renderComparisonResults(e)}catch(t){console.error(`Batch comparison failed:`,t),e.innerHTML=`
                <div class="text-center py-8 text-error font-body-sm">
        ...
        `,this.element=e,this.loadHistoryData(),e}async loadHistoryData(){let e=this.element.querySelector(`#history-runs-list`);try{let t=await O(Q,0);if(this.runsList=t.runs||[],this.total=t.total||this.runsList.length,e.innerHTML=``,this.runsList.length===0){e.innerHTML=`
                    <div class="text-center py-12 text-secondary font-body-sm">
