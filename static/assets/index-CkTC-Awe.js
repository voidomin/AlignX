(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`http://127.0.0.1:8000`,t=null;function n(e){t=e}function r(e={}){return t?{...e,"X-API-Key":t}:e}function i(e){return t?`${e}${e.includes(`?`)?`&`:`?`}api_key=${encodeURIComponent(t)}`:e}var a=/^\d[A-Z0-9]{3}$/,o=/^AF-[A-Z0-9]+-F\d+(-V\d+)?$/,s=/^SM-[A-Z0-9]+$/,c=/^ESM-MGYP\d+$/;function l(e){let t=(e||``).trim().toUpperCase();return a.test(t)||o.test(t)||s.test(t)||c.test(t)}async function u(){let t=await fetch(`${e}/health`,{priority:`low`});if(!t.ok)throw Error(`Health check failed`);return t.json()}async function d(t){let n=await fetch(`${e}/api/suggest?q=${encodeURIComponent(t)}`,{headers:r()});if(!n.ok)throw Error(`Suggestions fetch failed`);return n.json()}async function f(t){let n=await fetch(`${e}/api/chains`,{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:t})});if(!n.ok){let e=await n.json();throw Error(e.detail||`Chains fetch failed`)}return n.json()}async function p(t){let n=new FormData;n.append(`file`,t);let i=await fetch(`${e}/api/upload`,{method:`POST`,headers:r(),body:n});if(!i.ok){let e=await i.json();throw Error(e.detail||`Upload failed`)}return i.json()}async function m(t,n,i,a){let o=await fetch(`${e}/api/jobs/align`,{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:t,chain_selection:n,remove_water:i,remove_heteroatoms:a})});if(!o.ok){let e=await o.json();throw Error(e.detail||`Alignment submission failed`)}return o.json()}async function h(t){let n=await fetch(`${e}/api/jobs/${encodeURIComponent(t)}`,{headers:r()});if(!n.ok){let e=await n.json();throw Error(e.detail||`Job status fetch failed`)}return n.json()}async function g(e,{intervalMs:t=1500,onTick:n=null}={}){for(;;){let r=await h(e);if(n&&n(r),r.status===`completed`||r.status===`failed`)return r;await new Promise(e=>setTimeout(e,t))}}async function _(t,n){let i={pdb_id:t};n&&n.length>0&&(i.databases=n);let a=await fetch(`${e}/api/jobs/discover`,{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify(i)});if(!a.ok){let e=await a.json();throw Error(e.detail||`Discovery submission failed`)}return a.json()}async function v(t,n){let i=await fetch(`${e}/api/clusters`,{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({rmsd_df:t,threshold:n})});if(!i.ok){let e=await i.json();throw Error(e.detail||`Clusters fetch failed`)}return i.json()}async function y(t){let n=await fetch(`${e}/api/comparison/runs?exclude_run_id=${encodeURIComponent(t||``)}`,{headers:r()});if(!n.ok)throw Error(`Comparison runs fetch failed`);return n.json()}async function b(t,n){let i=await fetch(`${e}/api/comparison?current_run_id=${encodeURIComponent(t)}&target_run_id=${encodeURIComponent(n)}`,{headers:r()});if(!i.ok){let e=await i.json();throw Error(e.detail||`Comparison fetch failed`)}return i.json()}async function x(t,n){let i=await fetch(`${e}/api/ligands?pdb_id=${encodeURIComponent(t)}&run_id=${encodeURIComponent(n)}`,{headers:r()});if(!i.ok)throw Error(`Ligands fetch failed`);return i.json()}async function ee(t,n,i){let a=await fetch(`${e}/api/interactions?pdb_id=${encodeURIComponent(t)}&ligand_id=${encodeURIComponent(n)}&run_id=${encodeURIComponent(i)}`,{headers:r()});if(!a.ok)throw Error(`Interactions fetch failed`);return a.json()}async function S(){let t=await fetch(`${e}/api/memory`,{headers:r(),priority:`low`});if(!t.ok)throw Error(`Memory stats fetch failed`);return t.json()}async function C(){let t=await fetch(`${e}/api/memory/clear`,{method:`POST`,headers:r()});if(!t.ok)throw Error(`Clear memory execution failed`);return t.json()}async function w(t){let n=await fetch(`${e}/api/runs/${encodeURIComponent(t)}`,{headers:r()});if(!n.ok){let e=await n.json();throw Error(e.detail||`Run fetch failed`)}return n.json()}function T(e){return i(`${window.location.origin}/?shared_run=${encodeURIComponent(e)}`)}async function E(t=20,n=0){let i=await fetch(`${e}/api/history?limit=${t}&offset=${n}`,{headers:r()});if(!i.ok)throw Error(`History fetch failed`);return i.json()}async function D(){let t=await fetch(`${e}/api/stats`,{headers:r()});if(!t.ok)throw Error(`Stats fetch failed`);return t.json()}async function O(t){let n=await fetch(`${e}/api/sequence?run_id=${encodeURIComponent(t)}`,{headers:r()});if(!n.ok)throw Error(`Sequence alignment fetch failed`);return n.json()}function k(t){return i(`${e}/results/${encodeURIComponent(t)}/alignment.pdb`)}function A(t){return i(`${e}/results/${encodeURIComponent(t)}/alignment.fasta`)}function j(t,n){let r=`${e}/api/report?run_id=${encodeURIComponent(t)}`;return i(n&&n.length>0?`${r}&sections=${encodeURIComponent(n.join(`,`))}`:r)}function M(t){return i(`${e}/api/notebook?run_id=${encodeURIComponent(t)}`)}function N(t){return i(`${e}/api/discover/report?run_id=${encodeURIComponent(t)}`)}function P(t){return i(`${e}/api/discover/export?run_id=${encodeURIComponent(t)}`)}var F=[{key:`dashboard`,label:`Dashboard`},{key:`overview`,label:`Overview`},{key:`discover`,label:`Discover`},{key:`ligands`,label:`Ligands`},{key:`sequence`,label:`Sequence`},{key:`analytics`,label:`Analytics`},{key:`clusters`,label:`Clusters`},{key:`comparison`,label:`Compare`},{key:`history`,label:`History`}],I=class{constructor(e){this.onTabChange=e.onTabChange,this.onExportData=e.onExportData,this.onNewWorkspace=e.onNewWorkspace,this.activeTab=`overview`,this.element=null,this.memoryInterval=null}render(){let e=document.createElement(`header`);return e.className=`sticky top-0 z-50 bg-surface border-b border-border shrink-0`,e.innerHTML=`
        ...
        `;try{let{sequences:t,conservation:n}=await O(this.currentRunId),r=``,i={identity:`#ff4757`,high_similarity:`#ffa502`,gap:`#2f3542`,default:`transparent`};Object.keys(t).forEach(e=>{let a=t[e],o=``;for(let e=0;e<a.length;e++){let t=a[e],r=n[e],s=i.default;t===`-`?s=i.gap:r===1?s=i.identity:r>.7&&(s=i.high_similarity),o+=`<td class="${r>.5||t===`-`?`res-val`:``} text-center font-mono border border-border-subtle" style="background-color: ${s}; min-width: 22px; height: 24px; font-size: 12px; color: #fff;">${t}</td>`}r+=`
            ...
            `}}},G=[{key:`quality`,label:`Quality`},{key:`rmsf`,label:`RMSF`},{key:`rmsd`,label:`RMSD Matrix`},{key:`phylo`,label:`Phylogeny`}],K=class{element=null;currentRunId=null;heatmapFig=null;treeFig=null;ramachandranStats=null;rmsfValues=[];activeSubTab=`quality`;render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-analytics-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.setupSubTabs(),this.renderVisuals(),e}setupSubTabs(){this.element.querySelectorAll(`.analytics-subtab-btn`).forEach(e=>{e.addEventListener(`click`,()=>this.switchSubTab(e.dataset.subtab))}),this.updateSubTabView()}switchSubTab(e){this.activeSubTab=e,this.updateSubTabView();let t={rmsf:`rmsf-plotly-chart`,rmsd:`rmsd-plotly-heatmap`,phylo:`phylo-plotly-tree`}[e];if(t&&typeof Plotly<`u`){let e=this.element.querySelector(`#${t}`);e?.data&&Plotly.Plots.resize(e)}}updateSubTabView(){this.element.querySelectorAll(`.analytics-subtab-btn`).forEach(e=>{e.className=`analytics-subtab-btn flex-1 py-1.5 rounded-md font-label-md text-label-md transition-colors ${e.dataset.subtab===this.activeSubTab?`bg-accent-muted text-accent`:`text-secondary hover:text-primary`}`}),this.element.querySelectorAll(`[data-panel]`).forEach(e=>{e.classList.toggle(`hidden`,e.dataset.panel!==this.activeSubTab)})}updateResults(e,t,n,r,i){this.currentRunId=e,this.heatmapFig=t,this.treeFig=n,this.ramachandranStats=r,this.rmsfValues=i||[],this.renderVisuals()}renderVisuals(){if(!this.element)return;let e=this.element.querySelector(`#ramachandran-score`),t=this.element.querySelector(`#ramachandran-outliers`),n=this.element.querySelector(`#ramachandran-outliers-list-card`),r=this.element.querySelector(`#ramachandran-outliers-list`);this.ramachandranStats?.favored_percent==null?(e.innerText=`--`,t.innerText=`--`,n.classList.add(`hidden`)):(e.innerText=`${this.ramachandranStats.favored_percent.toFixed(1)}%`,t.innerText=this.ramachandranStats.outlier_count,this.ramachandranStats.outlier_count>0&&this.ramachandranStats.outliers_list?.length>0?(n.classList.remove(`hidden`),r.innerHTML=``,this.ramachandranStats.outliers_list.forEach(e=>{let t=document.createElement(`span`);t.className=`px-1.5 py-0.5 rounded-md bg-surface-raised border border-border-subtle text-error font-mono text-[10px]`,t.innerText=e,r.appendChild(t)})):n.classList.add(`hidden`));let i=this.element.querySelector(`#rmsf-plotly-chart`);if(this.rmsfValues?.length>0){i.innerHTML=``;let e={x:Array.from({length:this.rmsfValues.length},(e,t)=>t+1),y:this.rmsfValues,type:`scatter`,mode:`lines`,line:{color:`#E2846A`,width:2.5,shape:`spline`},fill:`tozeroy`,fillcolor:`rgba(226, 132, 106, 0.1)`,hoverinfo:`x+y`,name:`RMSF`};Plotly.newPlot(i,[e],{xaxis:{title:`Alignment Position`,gridcolor:`#2C2620`,zeroline:!1},yaxis:{title:`RMSF (Å)`,gridcolor:`#2C2620`,zeroline:!1},margin:{l:50,r:20,t:20,b:40},paper_bgcolor:`rgba(0,0,0,0)`,plot_bgcolor:`rgba(0,0,0,0)`,height:280,font:{family:`Segoe UI, sans-serif`,size:10,color:`#A79E8E`}},{responsive:!0,displayModeBar:!1})}else this.currentRunId&&(i.innerHTML=`
            ...
            `)}},q=class{rmsdDf=null;pdbMetadata={};threshold=3;element=null;debounceTimer=null;render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-clusters-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.setupEventListeners(),e}setupEventListeners(){this.element.querySelector(`#cluster-threshold-slider`).addEventListener(`input`,e=>{this.threshold=Number.parseFloat(e.target.value),this.element.querySelector(`#cluster-threshold-value`).innerText=`${this.threshold.toFixed(2)} Å`,clearTimeout(this.debounceTimer),this.debounceTimer=setTimeout(()=>this.loadClusters(),250)})}updateResults(e,t){this.rmsdDf=e,this.pdbMetadata=t||{},this.loadClusters()}async loadClusters(){if(!this.element)return;let e=this.element.querySelector(`#clusters-list-container`);if(!this.rmsdDf){e.innerHTML=`
            ...
            `}).join(``)}},J=class{currentRunId=null;pastRuns=[];targetRunId=null;element=null;render(){let e=document.createElement(`div`);return e.className=`flex-grow flex flex-col gap-4 overflow-y-auto pr-1`,e.id=`tab-comparison-container`,e.innerHTML=`
            <header class="section-head">
                <div>
