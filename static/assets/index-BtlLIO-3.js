(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`http://127.0.0.1:8000`;function t(e={}){return e}function n(e){return e}var r=/^[0-9][A-Z0-9]{3}$/,i=/^AF-[A-Z0-9]+-F[0-9]+(-V[0-9]+)?$/;function a(e){let t=(e||``).trim().toUpperCase();return r.test(t)||i.test(t)}async function o(){let t=await fetch(`${e}/health`);if(!t.ok)throw Error(`Health check failed`);return t.json()}async function s(n){let r=await fetch(`${e}/api/suggest?q=${encodeURIComponent(n)}`,{headers:t()});if(!r.ok)throw Error(`Suggestions fetch failed`);return r.json()}async function c(n){let r=await fetch(`${e}/api/chains`,{method:`POST`,headers:t({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:n})});if(!r.ok){let e=await r.json();throw Error(e.detail||`Chains fetch failed`)}return r.json()}async function l(n,r,i,a){let o=await fetch(`${e}/api/jobs/align`,{method:`POST`,headers:t({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:n,chain_selection:r,remove_water:i,remove_heteroatoms:a})});if(!o.ok){let e=await o.json();throw Error(e.detail||`Alignment submission failed`)}return o.json()}async function u(n){let r=await fetch(`${e}/api/jobs/${n}`,{headers:t()});if(!r.ok){let e=await r.json();throw Error(e.detail||`Job status fetch failed`)}return r.json()}async function d(e,{intervalMs:t=1500,onTick:n=null}={}){for(;;){let r=await u(e);if(n&&n(r),r.status===`completed`||r.status===`failed`)return r;await new Promise(e=>setTimeout(e,t))}}async function f(n,r){let i=await fetch(`${e}/api/clusters`,{method:`POST`,headers:t({"Content-Type":`application/json`}),body:JSON.stringify({rmsd_df:n,threshold:r})});if(!i.ok){let e=await i.json();throw Error(e.detail||`Clusters fetch failed`)}return i.json()}async function p(n){let r=await fetch(`${e}/api/comparison/runs?exclude_run_id=${encodeURIComponent(n||``)}`,{headers:t()});if(!r.ok)throw Error(`Comparison runs fetch failed`);return r.json()}async function m(n,r){let i=await fetch(`${e}/api/comparison?current_run_id=${encodeURIComponent(n)}&target_run_id=${encodeURIComponent(r)}`,{headers:t()});if(!i.ok){let e=await i.json();throw Error(e.detail||`Comparison fetch failed`)}return i.json()}async function h(n,r){let i=await fetch(`${e}/api/ligands?pdb_id=${n}&run_id=${r}`,{headers:t()});if(!i.ok)throw Error(`Ligands fetch failed`);return i.json()}async function g(n,r,i){let a=await fetch(`${e}/api/interactions?pdb_id=${n}&ligand_id=${r}&run_id=${i}`,{headers:t()});if(!a.ok)throw Error(`Interactions fetch failed`);return a.json()}async function _(){let n=await fetch(`${e}/api/memory`,{headers:t()});if(!n.ok)throw Error(`Memory stats fetch failed`);return n.json()}async function v(){let n=await fetch(`${e}/api/memory/clear`,{method:`POST`,headers:t()});if(!n.ok)throw Error(`Clear memory execution failed`);return n.json()}async function y(n=20,r=0){let i=await fetch(`${e}/api/history?limit=${n}&offset=${r}`,{headers:t()});if(!i.ok)throw Error(`History fetch failed`);return i.json()}async function b(){let n=await fetch(`${e}/api/stats`,{headers:t()});if(!n.ok)throw Error(`Stats fetch failed`);return n.json()}async function x(n){let r=await fetch(`${e}/api/sequence?run_id=${n}`,{headers:t()});if(!r.ok)throw Error(`Sequence alignment fetch failed`);return r.json()}function S(t){return`${e}/results/${t}/alignment.pdb`}function C(t){return`${e}/results/${t}/alignment.fasta`}function w(t,r){let i=`${e}/api/report?run_id=${t}`;return n(r&&r.length>0?`${i}&sections=${r.join(`,`)}`:i)}function T(t){return n(`${e}/api/notebook?run_id=${t}`)}var E=[{key:`dashboard`,label:`Dashboard`},{key:`overview`,label:`Overview`},{key:`ligands`,label:`Ligands`},{key:`sequence`,label:`Sequence`},{key:`analytics`,label:`Analytics`},{key:`clusters`,label:`Clusters`},{key:`comparison`,label:`Compare`},{key:`history`,label:`History`}],D=class{constructor(e){this.onTabChange=e.onTabChange,this.onExportData=e.onExportData,this.onNewWorkspace=e.onNewWorkspace,this.activeTab=`overview`,this.element=null,this.memoryInterval=null}render(){let e=document.createElement(`header`);return e.className=`sticky top-0 z-50 bg-surface border-b border-border shrink-0`,e.innerHTML=`
        ...
        `;try{let o=this.selectedPDBs[this.currentStructureIndex],s=(await g(o,e,this.currentRunId)).interactions,c=s.interactions;this.onLigandSelected(this.currentStructureIndex,e,c),n.innerText=`Conserved catalytic pocket near ligand ${s.ligand}. Stable hydrophobic cluster showing coordinated interactions.`,s.pocket_volume?(i.innerText=`Volume: ${s.pocket_volume.toFixed(1)} Å³`,i.classList.remove(`hidden`)):i.classList.add(`hidden`),s.pocket_sasa?(a.innerText=`SASA: ${s.pocket_sasa.toFixed(1)} Å²`,a.classList.remove(`hidden`)):a.classList.add(`hidden`),r.innerText=`${c.length} Found`,t.innerHTML=``,c.length===0?t.innerHTML=`
                    <tr>
                        <td colspan="5" class="text-center py-8 text-secondary font-body-sm">
        ...
        `;try{let{sequences:t,conservation:n}=await x(this.currentRunId),r=``,i={identity:`#ff4757`,high_similarity:`#ffa502`,gap:`#2f3542`,default:`transparent`};Object.keys(t).forEach(e=>{let a=t[e],o=``;for(let e=0;e<a.length;e++){let t=a[e],r=n[e],s=i.default;t===`-`?s=i.gap:r===1?s=i.identity:r>.7&&(s=i.high_similarity),o+=`<td class="${r>.5||t===`-`?`res-val`:``} text-center font-mono border border-border-subtle" style="background-color: ${s}; min-width: 22px; height: 24px; font-size: 12px; color: #fff;">${t}</td>`}r+=`
            ...
            `}}},F=[{key:`quality`,label:`Quality`},{key:`rmsf`,label:`RMSF`},{key:`rmsd`,label:`RMSD Matrix`},{key:`phylo`,label:`Phylogeny`}],I=class{constructor(){this.element=null,this.currentRunId=null,this.heatmapFig=null,this.treeFig=null,this.ramachandranStats=null,this.rmsfValues=[],this.activeSubTab=`quality`}render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-analytics-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.setupSubTabs(),this.renderVisuals(),e}setupSubTabs(){this.element.querySelectorAll(`.analytics-subtab-btn`).forEach(e=>{e.addEventListener(`click`,()=>this.switchSubTab(e.getAttribute(`data-subtab`)))}),this.updateSubTabView()}switchSubTab(e){this.activeSubTab=e,this.updateSubTabView();let t={rmsf:`rmsf-plotly-chart`,rmsd:`rmsd-plotly-heatmap`,phylo:`phylo-plotly-tree`}[e];if(t&&typeof Plotly<`u`){let e=this.element.querySelector(`#${t}`);e&&e.data&&Plotly.Plots.resize(e)}}updateSubTabView(){this.element.querySelectorAll(`.analytics-subtab-btn`).forEach(e=>{e.className=`analytics-subtab-btn flex-1 py-1.5 rounded-md font-label-md text-label-md transition-colors ${e.getAttribute(`data-subtab`)===this.activeSubTab?`bg-accent-muted text-accent`:`text-secondary hover:text-primary`}`}),this.element.querySelectorAll(`[data-panel]`).forEach(e=>{e.classList.toggle(`hidden`,e.getAttribute(`data-panel`)!==this.activeSubTab)})}updateResults(e,t,n,r,i){this.currentRunId=e,this.heatmapFig=t,this.treeFig=n,this.ramachandranStats=r,this.rmsfValues=i||[],this.renderVisuals()}renderVisuals(){if(!this.element)return;let e=this.element.querySelector(`#ramachandran-score`),t=this.element.querySelector(`#ramachandran-outliers`),n=this.element.querySelector(`#ramachandran-outliers-list-card`),r=this.element.querySelector(`#ramachandran-outliers-list`);this.ramachandranStats&&this.ramachandranStats.favored_percent!=null?(e.innerText=`${this.ramachandranStats.favored_percent.toFixed(1)}%`,t.innerText=this.ramachandranStats.outlier_count,this.ramachandranStats.outlier_count>0&&this.ramachandranStats.outliers_list&&this.ramachandranStats.outliers_list.length>0?(n.classList.remove(`hidden`),r.innerHTML=``,this.ramachandranStats.outliers_list.forEach(e=>{let t=document.createElement(`span`);t.className=`px-1.5 py-0.5 rounded-md bg-surface-raised border border-border-subtle text-error font-mono text-[10px]`,t.innerText=e,r.appendChild(t)})):n.classList.add(`hidden`)):(e.innerText=`--`,t.innerText=`--`,n.classList.add(`hidden`));let i=this.element.querySelector(`#rmsf-plotly-chart`);if(this.rmsfValues&&this.rmsfValues.length>0){i.innerHTML=``;let e={x:Array.from({length:this.rmsfValues.length},(e,t)=>t+1),y:this.rmsfValues,type:`scatter`,mode:`lines`,line:{color:`#E2846A`,width:2.5,shape:`spline`},fill:`tozeroy`,fillcolor:`rgba(226, 132, 106, 0.1)`,hoverinfo:`x+y`,name:`RMSF`};Plotly.newPlot(i,[e],{xaxis:{title:`Alignment Position`,gridcolor:`#2C2620`,zeroline:!1},yaxis:{title:`RMSF (Å)`,gridcolor:`#2C2620`,zeroline:!1},margin:{l:50,r:20,t:20,b:40},paper_bgcolor:`rgba(0,0,0,0)`,plot_bgcolor:`rgba(0,0,0,0)`,height:280,font:{family:`Segoe UI, sans-serif`,size:10,color:`#A79E8E`}},{responsive:!0,displayModeBar:!1})}else this.currentRunId&&(i.innerHTML=`
            ...
            `)}},L=class{constructor(){this.rmsdDf=null,this.pdbMetadata={},this.threshold=3,this.element=null,this.debounceTimer=null}render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-clusters-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.setupEventListeners(),e}setupEventListeners(){this.element.querySelector(`#cluster-threshold-slider`).addEventListener(`input`,e=>{this.threshold=parseFloat(e.target.value),this.element.querySelector(`#cluster-threshold-value`).innerText=`${this.threshold.toFixed(2)} Å`,clearTimeout(this.debounceTimer),this.debounceTimer=setTimeout(()=>this.loadClusters(),250)})}updateResults(e,t){this.rmsdDf=e,this.pdbMetadata=t||{},this.loadClusters()}async loadClusters(){if(!this.element)return;let e=this.element.querySelector(`#clusters-list-container`);if(!this.rmsdDf){e.innerHTML=`
            ...
            `;return}try{let e=await f(this.rmsdDf,this.threshold);this.renderClusters(e.clusters)}catch(t){console.error(`Failed to compute structural clusters:`,t),e.innerHTML=`
                <div class="text-center py-8 text-error font-body-sm">
            ...
            `}).join(``)}},R=class{constructor(){this.currentRunId=null,this.pastRuns=[],this.targetRunId=null,this.element=null}render(){let e=document.createElement(`div`);return e.className=`flex-grow flex flex-col gap-4 overflow-y-auto pr-1`,e.id=`tab-comparison-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `;try{let e=await m(this.currentRunId,this.targetRunId);this.renderComparisonResults(e)}catch(t){console.error(`Batch comparison failed:`,t),e.innerHTML=`
                <div class="text-center py-8 text-error font-body-sm">
        ...
        `,this.element=e,this.loadHistoryData(),e}async loadHistoryData(){let e=this.element.querySelector(`#history-runs-list`);try{let t=await y(z,0);if(this.runsList=t.runs||[],this.total=t.total||this.runsList.length,e.innerHTML=``,this.runsList.length===0){e.innerHTML=`
                    <div class="text-center py-12 text-secondary font-body-sm">
