(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`http://127.0.0.1:8000`;function t(e={}){return e}function n(e){return e}var r=/^[0-9][A-Z0-9]{3}$/,i=/^AF-[A-Z0-9]+-F[0-9]+(-V[0-9]+)?$/,a=/^SM-[A-Z0-9]+$/,o=/^ESM-MGYP[0-9]+$/;function s(e){let t=(e||``).trim().toUpperCase();return r.test(t)||i.test(t)||a.test(t)||o.test(t)}async function c(){let t=await fetch(`${e}/health`,{priority:`low`});if(!t.ok)throw Error(`Health check failed`);return t.json()}async function l(n){let r=await fetch(`${e}/api/suggest?q=${encodeURIComponent(n)}`,{headers:t()});if(!r.ok)throw Error(`Suggestions fetch failed`);return r.json()}async function u(n){let r=await fetch(`${e}/api/chains`,{method:`POST`,headers:t({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:n})});if(!r.ok){let e=await r.json();throw Error(e.detail||`Chains fetch failed`)}return r.json()}async function d(n,r,i,a){let o=await fetch(`${e}/api/jobs/align`,{method:`POST`,headers:t({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:n,chain_selection:r,remove_water:i,remove_heteroatoms:a})});if(!o.ok){let e=await o.json();throw Error(e.detail||`Alignment submission failed`)}return o.json()}async function f(n){let r=await fetch(`${e}/api/jobs/${n}`,{headers:t()});if(!r.ok){let e=await r.json();throw Error(e.detail||`Job status fetch failed`)}return r.json()}async function p(e,{intervalMs:t=1500,onTick:n=null}={}){for(;;){let r=await f(e);if(n&&n(r),r.status===`completed`||r.status===`failed`)return r;await new Promise(e=>setTimeout(e,t))}}async function m(n,r){let i={pdb_id:n};r&&r.length>0&&(i.databases=r);let a=await fetch(`${e}/api/jobs/discover`,{method:`POST`,headers:t({"Content-Type":`application/json`}),body:JSON.stringify(i)});if(!a.ok){let e=await a.json();throw Error(e.detail||`Discovery submission failed`)}return a.json()}async function h(n,r){let i=await fetch(`${e}/api/clusters`,{method:`POST`,headers:t({"Content-Type":`application/json`}),body:JSON.stringify({rmsd_df:n,threshold:r})});if(!i.ok){let e=await i.json();throw Error(e.detail||`Clusters fetch failed`)}return i.json()}async function g(n){let r=await fetch(`${e}/api/comparison/runs?exclude_run_id=${encodeURIComponent(n||``)}`,{headers:t()});if(!r.ok)throw Error(`Comparison runs fetch failed`);return r.json()}async function _(n,r){let i=await fetch(`${e}/api/comparison?current_run_id=${encodeURIComponent(n)}&target_run_id=${encodeURIComponent(r)}`,{headers:t()});if(!i.ok){let e=await i.json();throw Error(e.detail||`Comparison fetch failed`)}return i.json()}async function v(n,r){let i=await fetch(`${e}/api/ligands?pdb_id=${n}&run_id=${r}`,{headers:t()});if(!i.ok)throw Error(`Ligands fetch failed`);return i.json()}async function y(n,r,i){let a=await fetch(`${e}/api/interactions?pdb_id=${n}&ligand_id=${r}&run_id=${i}`,{headers:t()});if(!a.ok)throw Error(`Interactions fetch failed`);return a.json()}async function b(){let n=await fetch(`${e}/api/memory`,{headers:t(),priority:`low`});if(!n.ok)throw Error(`Memory stats fetch failed`);return n.json()}async function x(){let n=await fetch(`${e}/api/memory/clear`,{method:`POST`,headers:t()});if(!n.ok)throw Error(`Clear memory execution failed`);return n.json()}async function S(n=20,r=0){let i=await fetch(`${e}/api/history?limit=${n}&offset=${r}`,{headers:t()});if(!i.ok)throw Error(`History fetch failed`);return i.json()}async function C(){let n=await fetch(`${e}/api/stats`,{headers:t()});if(!n.ok)throw Error(`Stats fetch failed`);return n.json()}async function w(n){let r=await fetch(`${e}/api/sequence?run_id=${n}`,{headers:t()});if(!r.ok)throw Error(`Sequence alignment fetch failed`);return r.json()}function T(t){return`${e}/results/${t}/alignment.pdb`}function E(t){return`${e}/results/${t}/alignment.fasta`}function D(t,r){let i=`${e}/api/report?run_id=${t}`;return n(r&&r.length>0?`${i}&sections=${r.join(`,`)}`:i)}function O(t){return n(`${e}/api/notebook?run_id=${t}`)}var k=[{key:`dashboard`,label:`Dashboard`},{key:`overview`,label:`Overview`},{key:`discover`,label:`Discover`},{key:`ligands`,label:`Ligands`},{key:`sequence`,label:`Sequence`},{key:`analytics`,label:`Analytics`},{key:`clusters`,label:`Clusters`},{key:`comparison`,label:`Compare`},{key:`history`,label:`History`}],A=class{constructor(e){this.onTabChange=e.onTabChange,this.onExportData=e.onExportData,this.onNewWorkspace=e.onNewWorkspace,this.activeTab=`overview`,this.element=null,this.memoryInterval=null}render(){let e=document.createElement(`header`);return e.className=`sticky top-0 z-50 bg-surface border-b border-border shrink-0`,e.innerHTML=`
        ...
        `;try{let{sequences:t,conservation:n}=await w(this.currentRunId),r=``,i={identity:`#ff4757`,high_similarity:`#ffa502`,gap:`#2f3542`,default:`transparent`};Object.keys(t).forEach(e=>{let a=t[e],o=``;for(let e=0;e<a.length;e++){let t=a[e],r=n[e],s=i.default;t===`-`?s=i.gap:r===1?s=i.identity:r>.7&&(s=i.high_similarity),o+=`<td class="${r>.5||t===`-`?`res-val`:``} text-center font-mono border border-border-subtle" style="background-color: ${s}; min-width: 22px; height: 24px; font-size: 12px; color: #fff;">${t}</td>`}r+=`
            ...
            `}}},z=[{key:`quality`,label:`Quality`},{key:`rmsf`,label:`RMSF`},{key:`rmsd`,label:`RMSD Matrix`},{key:`phylo`,label:`Phylogeny`}],B=class{constructor(){this.element=null,this.currentRunId=null,this.heatmapFig=null,this.treeFig=null,this.ramachandranStats=null,this.rmsfValues=[],this.activeSubTab=`quality`}render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-analytics-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.setupSubTabs(),this.renderVisuals(),e}setupSubTabs(){this.element.querySelectorAll(`.analytics-subtab-btn`).forEach(e=>{e.addEventListener(`click`,()=>this.switchSubTab(e.getAttribute(`data-subtab`)))}),this.updateSubTabView()}switchSubTab(e){this.activeSubTab=e,this.updateSubTabView();let t={rmsf:`rmsf-plotly-chart`,rmsd:`rmsd-plotly-heatmap`,phylo:`phylo-plotly-tree`}[e];if(t&&typeof Plotly<`u`){let e=this.element.querySelector(`#${t}`);e&&e.data&&Plotly.Plots.resize(e)}}updateSubTabView(){this.element.querySelectorAll(`.analytics-subtab-btn`).forEach(e=>{e.className=`analytics-subtab-btn flex-1 py-1.5 rounded-md font-label-md text-label-md transition-colors ${e.getAttribute(`data-subtab`)===this.activeSubTab?`bg-accent-muted text-accent`:`text-secondary hover:text-primary`}`}),this.element.querySelectorAll(`[data-panel]`).forEach(e=>{e.classList.toggle(`hidden`,e.getAttribute(`data-panel`)!==this.activeSubTab)})}updateResults(e,t,n,r,i){this.currentRunId=e,this.heatmapFig=t,this.treeFig=n,this.ramachandranStats=r,this.rmsfValues=i||[],this.renderVisuals()}renderVisuals(){if(!this.element)return;let e=this.element.querySelector(`#ramachandran-score`),t=this.element.querySelector(`#ramachandran-outliers`),n=this.element.querySelector(`#ramachandran-outliers-list-card`),r=this.element.querySelector(`#ramachandran-outliers-list`);this.ramachandranStats&&this.ramachandranStats.favored_percent!=null?(e.innerText=`${this.ramachandranStats.favored_percent.toFixed(1)}%`,t.innerText=this.ramachandranStats.outlier_count,this.ramachandranStats.outlier_count>0&&this.ramachandranStats.outliers_list&&this.ramachandranStats.outliers_list.length>0?(n.classList.remove(`hidden`),r.innerHTML=``,this.ramachandranStats.outliers_list.forEach(e=>{let t=document.createElement(`span`);t.className=`px-1.5 py-0.5 rounded-md bg-surface-raised border border-border-subtle text-error font-mono text-[10px]`,t.innerText=e,r.appendChild(t)})):n.classList.add(`hidden`)):(e.innerText=`--`,t.innerText=`--`,n.classList.add(`hidden`));let i=this.element.querySelector(`#rmsf-plotly-chart`);if(this.rmsfValues&&this.rmsfValues.length>0){i.innerHTML=``;let e={x:Array.from({length:this.rmsfValues.length},(e,t)=>t+1),y:this.rmsfValues,type:`scatter`,mode:`lines`,line:{color:`#E2846A`,width:2.5,shape:`spline`},fill:`tozeroy`,fillcolor:`rgba(226, 132, 106, 0.1)`,hoverinfo:`x+y`,name:`RMSF`};Plotly.newPlot(i,[e],{xaxis:{title:`Alignment Position`,gridcolor:`#2C2620`,zeroline:!1},yaxis:{title:`RMSF (Å)`,gridcolor:`#2C2620`,zeroline:!1},margin:{l:50,r:20,t:20,b:40},paper_bgcolor:`rgba(0,0,0,0)`,plot_bgcolor:`rgba(0,0,0,0)`,height:280,font:{family:`Segoe UI, sans-serif`,size:10,color:`#A79E8E`}},{responsive:!0,displayModeBar:!1})}else this.currentRunId&&(i.innerHTML=`
            ...
            `)}},V=class{constructor(){this.rmsdDf=null,this.pdbMetadata={},this.threshold=3,this.element=null,this.debounceTimer=null}render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-clusters-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.setupEventListeners(),e}setupEventListeners(){this.element.querySelector(`#cluster-threshold-slider`).addEventListener(`input`,e=>{this.threshold=parseFloat(e.target.value),this.element.querySelector(`#cluster-threshold-value`).innerText=`${this.threshold.toFixed(2)} Å`,clearTimeout(this.debounceTimer),this.debounceTimer=setTimeout(()=>this.loadClusters(),250)})}updateResults(e,t){this.rmsdDf=e,this.pdbMetadata=t||{},this.loadClusters()}async loadClusters(){if(!this.element)return;let e=this.element.querySelector(`#clusters-list-container`);if(!this.rmsdDf){e.innerHTML=`
            ...
            `}).join(``)}},H=class{constructor(){this.currentRunId=null,this.pastRuns=[],this.targetRunId=null,this.element=null}render(){let e=document.createElement(`div`);return e.className=`flex-grow flex flex-col gap-4 overflow-y-auto pr-1`,e.id=`tab-comparison-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.loadHistoryData(),e}async loadHistoryData(){let e=this.element.querySelector(`#history-runs-list`);try{let t=await S(U,0);if(this.runsList=t.runs||[],this.total=t.total||this.runsList.length,e.innerHTML=``,this.runsList.length===0){e.innerHTML=`
                    <div class="text-center py-12 text-secondary font-body-sm">
        ...
        `,this.element=e,this.renderQuickStart(),this.loadDashboardData(),e}renderQuickStart(){let e=this.element.querySelector(`#dashboard-quick-start`);e.innerHTML=``,K.forEach(t=>{let n=document.createElement(`button`);n.className=`quick-start-btn px-3 py-1.5 rounded-md bg-surface-raised border border-border-subtle font-label-sm text-label-sm text-secondary hover:text-primary transition-colors`,n.textContent=`${t.label} (${t.pdbIds.join(` + `)})`,n.addEventListener(`click`,()=>this.onQuickStart(t.pdbIds)),e.appendChild(n)})}async loadDashboardData(){if(!this.element)return;try{let e=await C();this.element.querySelector(`#stat-total-runs`).textContent=e.total_runs,this.element.querySelector(`#stat-total-proteins`).textContent=e.total_proteins_analyzed,this.element.querySelector(`#stat-cache-size`).textContent=`${e.cache_size_mb} MB`}catch(e){console.error(`Failed to load dashboard stats:`,e)}let e=this.element.querySelector(`#dashboard-recent-runs`);try{let t=(await S(G,0)).runs||[];if(t.length===0){e.innerHTML=`
            ...
            `}}},J={pdb:`PDB`,alphafold:`AlphaFold`,swissmodel:`SWISS-MODEL`,esmfold:`ESMFold`},Y=[{key:`public`,label:`Public`},{key:`student`,label:`Student`},{key:`researcher`,label:`Researcher`}],X=class{constructor(){this.element=null,this.isRunning=!1,this.detailLevel=`student`,this.results=null}render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-discover-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,a=!n||n.annotated_neighbor_count===0?this.renderEmptyAnnotations(t):this.renderTieredBody(n);e.innerHTML=`
