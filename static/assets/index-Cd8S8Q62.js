(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`http://127.0.0.1:8000`,t=null;function n(e){t=e}function r(e={}){return t?{...e,"X-API-Key":t}:e}function i(t,n){let r=new URL(t,e);if(n)for(let[e,t]of Object.entries(n))t!=null&&r.searchParams.set(e,t);return r.toString()}function a(e){if(!t)return e;let n=new URL(e);return n.searchParams.set(`api_key`,t),n.toString()}var o=/^\d[A-Z0-9]{3}$/,s=/^AF-[A-Z0-9]+-F\d+(-V\d+)?$/,c=/^SM-[A-Z0-9]+$/,l=/^ESM-MGYP\d+$/;function u(e){let t=(e||``).trim().toUpperCase();return o.test(t)||s.test(t)||c.test(t)||l.test(t)}var d=/^[A-Za-z0-9_-]+$/;function f(e,t){if(typeof e!=`string`||!d.test(e))throw Error(`Invalid ${t}: ${JSON.stringify(e)}`);return e}function p(e,t){if(!u(e))throw Error(`Invalid ${t}: ${JSON.stringify(e)}`);return e}async function m(){let e=await fetch(i(`/health`),{priority:`low`});if(!e.ok)throw Error(`Health check failed`);return e.json()}async function ee(e){let t=await fetch(i(`/api/suggest`,{q:e}),{headers:r()});if(!t.ok)throw Error(`Suggestions fetch failed`);return t.json()}async function h(e){let t=await fetch(i(`/api/chains`),{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:e})});if(!t.ok){let e=await t.json();throw Error(e.detail||`Chains fetch failed`)}return t.json()}async function g(e){let t=new FormData;t.append(`file`,e);let n=await fetch(i(`/api/upload`),{method:`POST`,headers:r(),body:t});if(!n.ok){let e=await n.json();throw Error(e.detail||`Upload failed`)}return n.json()}async function _(e,t,n,a){let o=await fetch(i(`/api/jobs/align`),{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({pdb_ids:e,chain_selection:t,remove_water:n,remove_heteroatoms:a})});if(!o.ok){let e=await o.json();throw Error(e.detail||`Alignment submission failed`)}return o.json()}async function v(e){e=f(e,`jobId`);let t=await fetch(i(`/api/jobs/${e}`),{headers:r()});if(!t.ok){let e=await t.json();throw Error(e.detail||`Job status fetch failed`)}return t.json()}async function y(e,{intervalMs:t=1500,onTick:n=null}={}){for(;;){let r=await v(e);if(n&&n(r),r.status===`completed`||r.status===`failed`)return r;await new Promise(e=>setTimeout(e,t))}}async function b(e,t){let n={pdb_id:e};t&&t.length>0&&(n.databases=t);let a=await fetch(i(`/api/jobs/discover`),{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify(n)});if(!a.ok){let e=await a.json();throw Error(e.detail||`Discovery submission failed`)}return a.json()}async function x(e,t){let n=await fetch(i(`/api/clusters`),{method:`POST`,headers:r({"Content-Type":`application/json`}),body:JSON.stringify({rmsd_df:e,threshold:t})});if(!n.ok){let e=await n.json();throw Error(e.detail||`Clusters fetch failed`)}return n.json()}async function te(e){e=e?f(e,`excludeRunId`):``;let t=await fetch(i(`/api/comparison/runs`,{exclude_run_id:e}),{headers:r()});if(!t.ok)throw Error(`Comparison runs fetch failed`);return t.json()}async function ne(e,t){e=f(e,`currentRunId`),t=f(t,`targetRunId`);let n=await fetch(i(`/api/comparison`,{current_run_id:e,target_run_id:t}),{headers:r()});if(!n.ok){let e=await n.json();throw Error(e.detail||`Comparison fetch failed`)}return n.json()}async function S(e,t){e=p(e,`pdbId`),t=f(t,`runId`);let n=await fetch(i(`/api/ligands`,{pdb_id:e,run_id:t}),{headers:r()});if(!n.ok)throw Error(`Ligands fetch failed`);return n.json()}async function C(e,t,n){e=p(e,`pdbId`),t=f(t,`ligandId`),n=f(n,`runId`);let a=await fetch(i(`/api/interactions`,{pdb_id:e,ligand_id:t,run_id:n}),{headers:r()});if(!a.ok)throw Error(`Interactions fetch failed`);return a.json()}async function w(){let e=await fetch(i(`/api/memory`),{headers:r(),priority:`low`});if(!e.ok)throw Error(`Memory stats fetch failed`);return e.json()}async function T(){let e=await fetch(i(`/api/memory/clear`),{method:`POST`,headers:r()});if(!e.ok)throw Error(`Clear memory execution failed`);return e.json()}async function E(e){e=f(e,`runId`);let t=await fetch(i(`/api/runs/${e}`),{headers:r()});if(!t.ok){let e=await t.json();throw Error(e.detail||`Run fetch failed`)}return t.json()}function D(e){e=f(e,`runId`);let t=new URL(window.location.origin);return t.searchParams.set(`shared_run`,e),a(t.toString())}async function O(e=20,t=0){let n=await fetch(i(`/api/history`,{limit:e,offset:t}),{headers:r()});if(!n.ok)throw Error(`History fetch failed`);return n.json()}async function k(){let e=await fetch(i(`/api/stats`),{headers:r()});if(!e.ok)throw Error(`Stats fetch failed`);return e.json()}async function A(e){e=f(e,`runId`);let t=await fetch(i(`/api/sequence`,{run_id:e}),{headers:r()});if(!t.ok)throw Error(`Sequence alignment fetch failed`);return t.json()}function j(e){return e=f(e,`runId`),a(i(`/results/${e}/alignment.pdb`))}function M(e){return e=f(e,`runId`),a(i(`/results/${e}/alignment.fasta`))}var N=new Set([`summary`,`insights`,`heatmap`,`tree`,`matrix`]);function P(e,t){return e=f(e,`runId`),!t||t.length===0?a(i(`/api/report`,{run_id:e})):(t.forEach(e=>{if(!N.has(e))throw Error(`Invalid report section: ${JSON.stringify(e)}`)}),a(i(`/api/report`,{run_id:e,sections:t.join(`,`)})))}function F(e){return e=f(e,`runId`),a(i(`/api/notebook`,{run_id:e}))}function I(e){return e=f(e,`runId`),a(i(`/api/report/citations`,{run_id:e}))}function L(e){return e=f(e,`runId`),a(i(`/api/discover/report`,{run_id:e}))}function R(e){return e=f(e,`runId`),a(i(`/api/discover/export`,{run_id:e}))}function z(e){return e=f(e,`runId`),a(i(`/api/discover/citations`,{run_id:e}))}var B=[{key:`dashboard`,label:`Dashboard`},{key:`overview`,label:`Overview`},{key:`discover`,label:`Discover`},{key:`ligands`,label:`Ligands`},{key:`sequence`,label:`Sequence`},{key:`analytics`,label:`Analytics`},{key:`clusters`,label:`Clusters`},{key:`comparison`,label:`Compare`},{key:`history`,label:`History`}],V=class{constructor(e){this.onTabChange=e.onTabChange,this.onExportData=e.onExportData,this.onNewWorkspace=e.onNewWorkspace,this.activeTab=`overview`,this.element=null,this.memoryInterval=null}render(){let e=document.createElement(`header`);return e.className=`sticky top-0 z-50 bg-surface border-b border-border shrink-0`,e.innerHTML=`
        ...
        `,this.element=e,this.setupEventListeners(),this.refreshPDBList(),e}setupEventListeners(){let e=this.element.querySelector(`#add-pdb-btn`),t=this.element.querySelector(`#add-pdb-input`),n=this.element.querySelector(`#overview-run-btn`),r=this.element.querySelector(`#add-pdb-suggestions`),i=e=>{r.innerHTML=``,(e&&e.length>0?e.slice(0,4):[]).forEach(e=>{let n=document.createElement(`span`);n.className=`px-1.5 py-0.5 rounded-md bg-surface-raised border border-border-subtle font-label-sm text-label-sm text-secondary cursor-pointer hover:text-primary transition-colors`,n.innerText=e,n.addEventListener(`click`,()=>{this.onAddPDB(e),t.value=``,i([])}),r.appendChild(n)})};t.addEventListener(`input`,()=>{clearTimeout(this.suggestTimeout);let e=t.value.trim();if(e.length<1){i([]);return}this.suggestTimeout=setTimeout(async()=>{try{i((await ee(e)).suggestions)}catch(e){console.error(`Autocomplete suggestions failed:`,e)}},300)}),e.addEventListener(`click`,()=>{let e=t.value.trim().toUpperCase();u(e)&&(this.onAddPDB(e),t.value=``,i([]))}),t.addEventListener(`keypress`,e=>{if(e.key===`Enter`){let e=t.value.trim().toUpperCase();u(e)&&(this.onAddPDB(e),t.value=``,i([]))}}),n.addEventListener(`click`,()=>{this.onRunAlignment()});let a=this.element.querySelector(`#toggle-batch-add-btn`),o=this.element.querySelector(`#batch-add-container`),s=this.element.querySelector(`#batch-pdb-input`),c=this.element.querySelector(`#batch-add-btn`),l=this.element.querySelector(`#batch-add-feedback`);a.addEventListener(`click`,()=>{this.batchInputVisible=!this.batchInputVisible,o.classList.toggle(`hidden`,!this.batchInputVisible),this.batchInputVisible&&s.focus()}),c.addEventListener(`click`,async()=>{let e=s.value.split(/[\s,]+/).map(e=>e.trim().toUpperCase()).filter(Boolean),t=[],n=[],r=0,i=new Set(this.selectedPDBs);e.forEach(e=>{if(!u(e)){n.push(e);return}if(i.has(e)){r+=1;return}i.add(e),t.push(e)});let a=0,o=0;if(t.length>0){let e=await this.onAddManyPDBs(t);o=e?.added?e.added.length:t.length,a=e?.overCap||0}let c=[];o>0&&c.push(`Added ${o}.`),r>0&&c.push(`Skipped ${r} already in the workspace.`),n.length>0&&c.push(`Couldn't recognize: ${n.join(`, `)}.`),a>0&&c.push(`Skipped ${a} — workspace limit is 20 structures.`),c.length===0&&c.push(`Nothing to add — paste at least one ID.`),l.innerText=c.join(` `),o>0&&(s.value=``)});let d=this.element.querySelector(`#upload-structure-btn`),f=this.element.querySelector(`#upload-structure-input`),p=this.element.querySelector(`#upload-structure-feedback`);d.addEventListener(`click`,()=>f.click()),f.addEventListener(`change`,async()=>{let e=f.files?.[0];if(f.value=``,e){this.isUploading=!0,p.innerText=`Uploading ${e.name}...`;try{await this.onUploadStructure(e),p.innerText=`Added ${e.name}.`}catch(t){p.innerText=t.message||`Upload of ${e.name} failed.`}finally{this.isUploading=!1}}})}updateState(e,t,n){this.selectedPDBs=e,this.chainSelections=t,this.pdbMetadata=n,this.refreshPDBList()}setLoadingChains(e){this.isLoadingChains=e,this.refreshPDBList();let t=this.element?.querySelector(`#overview-run-btn`);t&&(t.disabled=e)}refreshPDBList(){if(!this.element)return;let e=this.element.querySelector(`#pdb-count-badge`);e.innerText=`${this.selectedPDBs.length} Protein${this.selectedPDBs.length===1?``:`s`}`;let t=this.element.querySelector(`#pdb-list-container`);if(this.isLoadingChains){t.innerHTML=`
            ...
            `;let e=t.querySelector(`#overview-quick-start`);e&&this.onQuickStart&&G.forEach(t=>{let n=document.createElement(`button`);n.type=`button`,n.className=`quick-start-btn px-3 py-1.5 rounded-md bg-surface-raised border border-border-subtle font-label-sm text-label-sm text-secondary hover:text-primary transition-colors`,n.textContent=`${t.label} (${t.pdbIds.join(` + `)})`,n.addEventListener(`click`,()=>this.onQuickStart(t.pdbIds)),e.appendChild(n)});return}this.selectedPDBs.forEach(e=>{let n=this.pdbMetadata[e],r=document.createElement(`div`);r.className=`flex flex-col gap-1.5 p-3 rounded-md bg-surface-raised border border-border-subtle`;let i=``;n?.chains?n.chains.forEach(t=>{let n=this.chainSelections[e]===t.id?`selected`:``;i+=`<option value="${t.id}" ${n}>Chain ${t.id} (${t.residue_count} residues)</option>`}):i=`<option value="A">Chain A</option>`;let a=K[n?.source]||`PDB`,o=n?[n.method,n.resolution,n.organism].filter(e=>e&&e!==`N/A`):[];n?.source===`upload`&&n.original_filename&&o.push(W(n.original_filename)),r.innerHTML=`
        ...
        `;try{let o=this.selectedPDBs[this.currentStructureIndex],s=(await C(o,e,this.currentRunId)).interactions,c=s.interactions;this.onLigandSelected(this.currentStructureIndex,e,c),n.innerText=`Conserved catalytic pocket near ligand ${s.ligand}. Stable hydrophobic cluster showing coordinated interactions.`,s.pocket_volume?(i.innerText=`Volume: ${s.pocket_volume.toFixed(1)} Å³`,i.classList.remove(`hidden`)):i.classList.add(`hidden`),s.pocket_sasa?(a.innerText=`SASA: ${s.pocket_sasa.toFixed(1)} Å²`,a.classList.remove(`hidden`)):a.classList.add(`hidden`),r.innerText=`${c.length} Found`,t.innerHTML=``,c.length===0?t.innerHTML=`
                    <tr>
                        <td colspan="5" class="text-center py-8 text-secondary font-body-sm">
        ...
        `;try{let{sequences:t,conservation:n}=await A(this.currentRunId),r=``,i={identity:`#ff4757`,high_similarity:`#ffa502`,gap:`#2f3542`,default:`transparent`};Object.keys(t).forEach(e=>{let a=t[e],o=``;for(let e=0;e<a.length;e++){let t=a[e],r=n[e],s=i.default;t===`-`?s=i.gap:r===1?s=i.identity:r>.7&&(s=i.high_similarity),o+=`<td class="${r>.5||t===`-`?`res-val`:``} text-center font-mono border border-border-subtle" style="background-color: ${s}; min-width: 22px; height: 24px; font-size: 12px; color: #fff;">${t}</td>`}r+=`
            ...
            `}}},ie=[{key:`quality`,label:`Quality`},{key:`rmsf`,label:`RMSF`},{key:`rmsd`,label:`RMSD Matrix`},{key:`phylo`,label:`Phylogeny`},{key:`insights`,label:`Insights`}],ae=class{element=null;currentRunId=null;heatmapFig=null;treeFig=null;ramachandranStats=null;rmsfValues=[];insights=[];activeSubTab=`quality`;render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-analytics-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.setupSubTabs(),this.renderVisuals(),e}setupSubTabs(){this.element.querySelectorAll(`.analytics-subtab-btn`).forEach(e=>{e.addEventListener(`click`,()=>this.switchSubTab(e.dataset.subtab))}),this.updateSubTabView()}switchSubTab(e){this.activeSubTab=e,this.updateSubTabView();let t={rmsf:`rmsf-plotly-chart`,rmsd:`rmsd-plotly-heatmap`,phylo:`phylo-plotly-tree`}[e];if(t&&typeof Plotly<`u`){let e=this.element.querySelector(`#${t}`);e?.data&&Plotly.Plots.resize(e)}}updateSubTabView(){this.element.querySelectorAll(`.analytics-subtab-btn`).forEach(e=>{e.className=`analytics-subtab-btn flex-1 py-1.5 rounded-md font-label-md text-label-md transition-colors ${e.dataset.subtab===this.activeSubTab?`bg-accent-muted text-accent`:`text-secondary hover:text-primary`}`}),this.element.querySelectorAll(`[data-panel]`).forEach(e=>{e.classList.toggle(`hidden`,e.dataset.panel!==this.activeSubTab)})}updateResults(e,t,n,r,i,a){this.currentRunId=e,this.heatmapFig=t,this.treeFig=n,this.ramachandranStats=r,this.rmsfValues=i||[],this.insights=a||[],this.renderVisuals()}renderVisuals(){if(!this.element)return;let e=this.element.querySelector(`#ramachandran-score`),t=this.element.querySelector(`#ramachandran-outliers`),n=this.element.querySelector(`#ramachandran-outliers-list-card`),r=this.element.querySelector(`#ramachandran-outliers-list`);this.ramachandranStats?.favored_percent==null?(e.innerText=`--`,t.innerText=`--`,n.classList.add(`hidden`)):(e.innerText=`${this.ramachandranStats.favored_percent.toFixed(1)}%`,t.innerText=this.ramachandranStats.outlier_count,this.ramachandranStats.outlier_count>0&&this.ramachandranStats.outliers_list?.length>0?(n.classList.remove(`hidden`),r.innerHTML=``,this.ramachandranStats.outliers_list.forEach(e=>{let t=document.createElement(`span`);t.className=`px-1.5 py-0.5 rounded-md bg-surface-raised border border-border-subtle text-error font-mono text-[10px]`,t.innerText=e,r.appendChild(t)})):n.classList.add(`hidden`));let i=this.element.querySelector(`#rmsf-plotly-chart`);if(this.rmsfValues?.length>0){i.innerHTML=``;let e={x:Array.from({length:this.rmsfValues.length},(e,t)=>t+1),y:this.rmsfValues,type:`scatter`,mode:`lines`,line:{color:`#E2846A`,width:2.5,shape:`spline`},fill:`tozeroy`,fillcolor:`rgba(226, 132, 106, 0.1)`,hoverinfo:`x+y`,name:`RMSF`};Plotly.newPlot(i,[e],{xaxis:{title:`Alignment Position`,gridcolor:`#2C2620`,zeroline:!1},yaxis:{title:`RMSF (Å)`,gridcolor:`#2C2620`,zeroline:!1},margin:{l:50,r:20,t:20,b:40},paper_bgcolor:`rgba(0,0,0,0)`,plot_bgcolor:`rgba(0,0,0,0)`,height:280,font:{family:`Segoe UI, sans-serif`,size:10,color:`#A79E8E`}},{responsive:!0,displayModeBar:!1})}else this.currentRunId&&(i.innerHTML=`
            ...
            `);let s=this.element.querySelector(`#analytics-insights-list`),c=this.element.querySelector(`#analytics-insights-empty`);s.innerHTML=``,this.insights?.length>0?(c.classList.add(`hidden`),this.insights.forEach(e=>{let t=document.createElement(`li`);t.className=`font-body-sm text-primary border border-border-subtle rounded-md p-2`,t.innerHTML=W(e).replace(/\*\*(.+?)\*\*/g,`<strong>$1</strong>`),s.appendChild(t)})):this.currentRunId?(c.textContent=`No automated insights available for this run.`,c.classList.remove(`hidden`)):(c.textContent=`Run alignment to display automated insights.`,c.classList.remove(`hidden`))}},oe=class{rmsdDf=null;pdbMetadata={};threshold=3;element=null;debounceTimer=null;render(){let e=document.createElement(`div`);return e.className=`editorial-section`,e.id=`tab-clusters-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `,this.element=e,this.setupEventListeners(),e}setupEventListeners(){this.element.querySelector(`#cluster-threshold-slider`).addEventListener(`input`,e=>{this.threshold=Number.parseFloat(e.target.value),this.element.querySelector(`#cluster-threshold-value`).innerText=`${this.threshold.toFixed(2)} Å`,clearTimeout(this.debounceTimer),this.debounceTimer=setTimeout(()=>this.loadClusters(),250)})}updateResults(e,t){this.rmsdDf=e,this.pdbMetadata=t||{},this.loadClusters()}async loadClusters(){if(!this.element)return;let e=this.element.querySelector(`#clusters-list-container`);if(!this.rmsdDf){e.innerHTML=`
            ...
            `;return}try{let e=await x(this.rmsdDf,this.threshold);this.renderClusters(e.clusters)}catch(t){console.error(`Failed to compute structural clusters:`,t),e.innerHTML=`
                <div class="text-center py-8 text-error font-body-sm">
            ...
            `}).join(``)}},se=class{currentRunId=null;pastRuns=[];targetRunId=null;element=null;render(){let e=document.createElement(`div`);return e.className=`flex-grow flex flex-col gap-4 overflow-y-auto pr-1`,e.id=`tab-comparison-container`,e.innerHTML=`
            <header class="section-head">
                <div>
        ...
        `;try{let e=await ne(this.currentRunId,this.targetRunId);this.renderComparisonResults(e)}catch(t){console.error(`Batch comparison failed:`,t),e.innerHTML=`
                <div class="text-center py-8 text-error font-body-sm">
        ...
        `,this.element=e,this.loadHistoryData(),e}async loadHistoryData(){let e=this.element.querySelector(`#history-runs-list`);try{let t=await O(Z,0);if(this.runsList=t.runs||[],this.total=t.total||this.runsList.length,e.innerHTML=``,this.runsList.length===0){e.innerHTML=`
                    <div class="text-center py-12 text-secondary font-body-sm">
        ...
        `,a=this.renderBody(t,n),o=t.id?`
            <div class="flex gap-4">
                <a href="${L(t.id)}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-1 font-label-sm text-label-sm text-secondary hover:text-primary transition-colors">
                    <span class="material-symbols-outlined text-[16px]">description</span>
                    Download Report
                </a>
                <a href="${R(t.id)}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-1 font-label-sm text-label-sm text-secondary hover:text-primary transition-colors">
                    <span class="material-symbols-outlined text-[16px]">data_object</span>
                    Download JSON
                </a>
                <a href="${z(t.id)}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-1 font-label-sm text-label-sm text-secondary hover:text-primary transition-colors">
